...
lint: vet ## Vet + shellcheck the shell components (if available)
	@command -v shellcheck >/dev/null 2>&1 && shellcheck install.sh tmon.tmux scripts/bootstrap.sh scripts/bump-version.sh cmd/tmon/hooks/*.sh || echo "shellcheck not installed; skipping"
